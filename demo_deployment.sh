...
echo "4. Run ./deploy_bot.sh - everything else is automatic!"
echo ""

if [ -f ".env" ]; then
    echo "Current .env status:"
    grep "^[A-Z]" .env 2>/dev/null || echo "No active configurations found"
    echo ""
    ...
    echo "No .env file found. Run 'python3 detect_config.py' to create one."
fi
